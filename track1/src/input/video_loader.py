...
class VideoLoader:
    ...
    def __init__(self, video_path: str | Path) -> None:
        ...
        if self._frame_count <= 0:
            logger.warning("Video reports zero frames.")

        self._duration = (self._frame_count / self._fps)

        # Video Codec stored as int
        fourcc = int(self._capture.get(cv2.CAP_PROP_FOURCC)) 
        # codec converted to human-readable characters
        self._codec = "".join(chr((fourcc >> (8 * i)) & 0xFF) for i in range(4)).strip() 
    ...
    def read(self) -> tuple[bool, np.ndarray | None]:
        """Read the next frame & return (success, frame)"""
    ...
    def get_frame(self, frame_index: int) -> np.ndarray | None:
        """Read a specific frame without affecting current playback position"""
        ...
        if not success:
            logger.warning(f"Unable to read frame {frame_index}")
            return None
        return frame
    ...
    def __iter__(self) -> Iterator[tuple[int, np.ndarray]]:
        """Prepares to iterate through frames of a video."""
        self.reset()
        return self
    ...
    def __del__(self) -> None:
        ...
        try:
            ...
            # Never allow exceptions to escape during interpreter shutdown.
            pass