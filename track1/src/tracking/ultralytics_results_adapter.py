...
class TrackingResults:
    ...
    def __getitem__(self, index) -> "TrackingResults":
    ...
    # Factory
    @classmethod
    def from_detections(cls, detections) -> "TrackingResults":
        if not detections:
            return cls(
        ...
        class_id = np.asarray([detection.class_id for detection in detections], dtype=np.float32)
        return cls(xyxy=xyxy, conf=confidence, cls=class_id)