...
class DeepSORTTracker(BaseTracker):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._tracker = DeepSort(
            ...
            n_init=tracking_cfg["n_init"],
            max_iou_distance=tracking_cfg["max_iou_distance"],
            max_cosine_distance=tracking_cfg["max_cosine_distance"],
        )
    ...
    def update(self, detections: list[Detection], frame: np.ndarray | None = None) -> list[Track]:
        ...
        if frame is None:
            raise ValueError("DeepSORT requires the current frame.")

        class_lookup = {detection.class_name: detection for detection in detections}
        ds_detections = []
        ...
        for tracked in tracked_objects:
            ...
            x1, y1, x2, y2 = (tracked.to_ltrb())
            class_name = getattr(tracked, "det_class", "unknown")
            detection = class_lookup.get(class_name)
            class_id = (detection.class_id if detection is not None else -1)
